	...
	: QTableWidget(parent)
{
	...
	variablesTable->setColumnCount(4);
	variablesTable->setRowCount(10);
...
void VariablesViewer::updateTable()
{
	...
	// filter to variables and values only (no jump address labels)
	...
	for (int i = 0; i < variables.size(); i += 1) {
		...
		variablesTable->setItem(i, 1, address);
...
void VariablesViewer::updateVariableValue(int variable_id, QString value)
{
	...
	variablesTable->setItem(variable_id, 2, value_item);
}
